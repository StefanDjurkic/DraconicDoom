...
namespace Helper_Functions
{
	...
	int Confirmation(string spec)
	{
		...
		{
			cout << "You chose " << spec << ". Do you accept?" << endl;               // Display our input string
		...
		return exitFunction;                                                          // If confirm was yes then we return exitfunction (66)
	}
	...
	{
		cout << "|";																// Border opening							
		for (int i = 0; i < txtLength; i++)                                         // For the specified length (txtLength) we cout the character (spacing)
		{
			cout << spacing;
		}
	...
	void MakeBorder(int txtLength, const char * innerSpacing, const char * outerSpacing, const char * phrase, int lineAmt)
	{
		...
		cout << "|"; // Border opening
		int testPhrase = strlen(phrase);                // Measure the length of our phrase here and store it in int testPhrase
		txtLength -= testPhrase;						// Find how much space we desire by subtracting the length of testPhrase
		...
		/* Below we check if lineAmt is greater than one to print an additional border, if lineAmt is -1 we only print a border here and not at the start of the function */
	...
	/*This function takes in two integer parameters as a range and returns a random value within the range*/
	int RandomGen(int range1, int range2)
	{
		...
		std::uniform_int_distribution<> uniDis(range1, range2);				// Create a distribution based on our ranges
	...
	int CinCheck()
	{
		...
		cin >> rPick;														// Request input into rpick*/
		...
		while (cin.fail() || rPick == exitFunction) 
		{
			...
			rPick = 0;														// Reset rPick to zero to be returned which should indicate that something has failed
		}
		return rPick;														// Return our value
	}

	// this function shows formatted text based on the position of the player and returns the corresponding string
	std::string ShowBanner(int storyPoint)
	{
		std::string areaResult = "Village"; // set some default value just in-case
		// This is our game title we will likely place this in some other function later on
		if (storyPoint == 0) 
		{
			/* Use makeborder function to create some ascii art*/
		...
		// Check the storypoint integer and based on these index display some ascii art
		if (storyPoint == 1) {
			MakeBorder(80, " ", "=", ".::         .::      .::  .::                                ", 1);
			MakeBorder(80, " ", "", " .::       .::   .:  .::  .::                                ", 0);
			MakeBorder(80, " ", "", "  .::     .::        .::  .::    .::        .::       .::    ", 0);
