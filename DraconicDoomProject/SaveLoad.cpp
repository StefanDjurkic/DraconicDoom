...
using namespace Helper_Functions;
	namespace Save_Load
	{
		...
		void Save_Load::Save2dVector(const char * file2d, std::vector <std::vector <int> > saveVect2d)
		{
			...
			{
				std::ofstream outfile(file2d);								// note: Consider removing/altering
		...
		{
			std::ifstream fin2d(file2d);									// Declare an ifstream object and have it open the passed parameter
			...
			int value;														// Variable which is used to pass in a split from the vectorLine object
				while (std::getline(fin2d, vectorLine))						// While we're getting a line into our string object vectorLine
				{
					...
					while (split >> value)									// While split is being inputted into value
					{
						/* Address the object with back() which puts us at the last element in the vector, then push_back with value*/
						loadedVector.back().push_back(value);				// Pass into our loadedVector the value variable
					}
				}
				fin2d.close();
			}
			return loadedVector;											// Return the newly built 2dvector*/
		};
	}